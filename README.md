...
. Hit 'Generate access token' and it will produce one in the box. This is the access token you need to put in the URL in the code where it says 'YOUR-ACCESS-TOKEN'. This token is a 'short lived' token and will only work for one hour. If you'd like one that works for longer, Facebook will extend it if you head over to Tools > Access Token Debugger, paste in your new token, press 'Debug' and scroll until you see 'Extend Acess Token'. Hit that and it should extend it for two months! Be prepared to have to re-fresh this token after the alloted time though...
